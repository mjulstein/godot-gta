...
@onready var vehicle: CharacterBody2D = $CivilianVehicle
...
func _ready() -> void:
	...
	interaction_system.configure(player, [vehicle])
...
func _process(_delta: float) -> void:
	...
	debug_state.set_player_mode("Driving" if player.is_in_vehicle() else "On Foot")
