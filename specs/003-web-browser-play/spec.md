...
This also leaves browser distribution underspecified. If the team wants a quick playable web build on GitHub Pages, the repository needs a defined publish workflow that can push exported web artifacts to a dedicated publish branch without tracking those built files on the working branch.
...
5. **Given** the game is running on a platform or runtime mode where a non-fullscreen display mode does not exist, **When** the pause overlay is shown, **Then** the fullscreen toggle control is not shown.
...
- **FR-016**: The implementation SHOULD prefer Godot-native window-mode APIs for fullscreen toggling, while respecting browser limitations on when fullscreen requests are allowed.
- **FR-017**: If a GitHub Pages publish helper is provided for this feature, it MUST publish exported web artifacts without requiring those built files to be tracked on the working branch.
- **FR-018**: If a GitHub Pages publish helper is provided for this feature, it SHOULD be able to publish a site from a dedicated branch that keeps the playable files under `docs/`.
- **FR-019**: If a GitHub Pages publish helper is provided for this feature, it MUST return the repository to the branch that was checked out when the publish command started, rather than assuming a fixed branch name.

### Non-Functional Requirements

- **NFR-001**: Browser play SHOULD feel functionally equivalent to desktop for the current sandbox slice, with differences limited to unavoidable platform constraints.
- **NFR-002**: The browser feature SHOULD be implemented as a narrow export-platform layer or pause-flow extension rather than a separate gameplay fork.
...
- **A-004**: "1:1" parity means same gameplay behavior, controls, pause flow, and framing intent, not guaranteed identical performance on every browser or machine.
...
- **OOS-007**: A full release pipeline beyond a simple manual publish helper for GitHub Pages-style hosting.
...
- If direct fullscreen toggling from Godot input proves unreliable in web exports, use a custom HTML5 shell and a narrow JavaScript bridge that calls the browser fullscreen API from the active browser input event path.
- Prefer the custom-shell workaround only for the browser fullscreen boundary, while keeping gameplay logic and pause state inside Godot.
...
- If a publish helper is added, prefer a manual script that exports to a temporary directory and force-pushes a dedicated publish branch rather than checking built web artifacts into the working branch.
- If a publish helper is added, it should capture the starting branch before publishing and restore that same branch when the publish flow finishes.
...
- **SC-008**: If a manual GitHub Pages publish helper is implemented, it can publish the current web build to the designated publish branch without leaving exported web artifacts tracked on the working branch.
- **SC-009**: If a manual GitHub Pages publish helper is implemented, it returns the repository to the same branch that was active before the publish command began.
